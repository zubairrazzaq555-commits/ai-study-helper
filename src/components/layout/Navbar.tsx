...
import { BookOpen, LayoutDashboard, Menu, X, Sparkles } from "lucide-react";
      ...
      <div className="mx-4 mt-4">
        <div className="glass-strong rounded-2xl px-5 py-3 flex items-center justify-between max-w-6xl mx-auto">
          ...
          <div className="hidden md:flex items-center gap-2">
            ...
            <Link href="/notes"
              className="px-4 py-2 rounded-xl bg-indigo-600 hover:bg-indigo-500 text-white text-sm font-semibold transition-all duration-200 glow-accent-hover">
              {t.nav.tryFree}
            </Link>
          </div>
          ...
          <div className="glass-strong rounded-2xl p-4 space-y-2">
            ...
            <div className="flex gap-2">
              <button onClick={() => setLang(lang === "en" ? "zh" : "en")}
                className="flex-1 flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl border border-white/10 text-zinc-400 text-sm font-medium">
                <span>{lang === "en" ? "🇨🇳" : "🇬🇧"}</span>
                {lang === "en" ? "中文" : "English"}
              </button>
